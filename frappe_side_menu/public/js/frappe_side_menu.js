...
// _______________________________________x_____________X____________x____________________________________________
    ...
    function updateRecordList(records) {
        ...
        records.forEach(function(record) {
            ...
            // No need to observe each listItem individually, observe the entire list once
            hidemainMenu();
        });
    
        // Observe the entire list after all list items are added
    ...
    function handleFormRefresh(frm) {
        try {
            ...
            if (frm && frm.doc && frm.doc.doctype && frm.doc.doctype !== "DocType") {
                ...
                if (currentPage !== newDoctype) {
                    currentPage = newDoctype;
                    filterEvent()
                    clearRecords();
                    loadMoreRecords(doctype, recordListContainer);
                }
...
// _______________________________________x_____________X____________x____________________________________________
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('infoContainer')) {
            const link = event.target.querySelector('.recordLink');
            const href = link ? link.getAttribute('href') : null;
